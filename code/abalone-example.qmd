...
par(mfrow = c(2, 2))   # making 4 panels
plot(abalone_model)    # displaying plot
...
**2.98 +/- 0.76 (SE) With each increase in pH, the model predicts an increase of 2.98 +/- 0.76 (SE) inchange in shell surface area per day (mm^2^ d^-1^). **  

What is the intercept estimate (with standard error?)  

**-18.5 +/- 6.16 (SE); The model predicts that abalone change in shell surface area per day is -18.5 +/- 6.16 (SE). (note to self: this is not necessarily biologically realistic but is something the model generates as an estimate.**
...
**x - predictor variable values for pH; predicted - model predictions for change in shell surface area per day; SE - for standard error, which is the precision of the prediction; conf. low and conf. high - range for the 95% confidence interval**  
